...
export const projects = $state<{ value: Array<LocalProject> }>({ value: [] });
export const currentProject = $state<{ value: LocalProject | null }>({
  value: null,
});

// The current task is used to determine the data shown in the edit-popup.
export const currentTask = $state<{ value: LocalProjectTask | null }>({
  value: null,
});
...
export function update() {
  $effect(() => {
    async function sett() {
      ...
      let newCustomFields: Array<LocalProjectCustomField> = [];
      for (let item of currentProject.value.customFields) {
      ...
      let newTasks: Array<LocalProjectTask> = [];
      for (let item of currentProject.value.tasks) {
